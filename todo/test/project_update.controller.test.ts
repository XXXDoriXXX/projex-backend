...
import { updateProject } from '../../src/controllers/project.controller';
...
const notValidtestToken = jwt.sign({ userId: 'cmfc9n2l70000c6g0i1wd6t4u', username: 'Test' }, JWT_SECRET, { expiresIn: '1h' });
...
describe('PUT /api/project/:id', () => {
    ...
    it('should update error project', async () => {
        const res = await request(app).put('/api/project/cmf1e5bu50001c62s84jw0nq7').set('Authorization', `Bearer ${notValidtestToken}`).send({
            title: 'Test Project3',
            description: 'This is a test project',
        });
    ...
    it('should update error project', async () => {
        const res = await request(app).put('/api/project/cmf1e5bu50001c62s84jw0n7').set('Authorization', `Bearer ${testToken}`).send({
            title: 'Test Project3',
            description: 'This is a test project',
        });
        expect(res.status).toBe(400);
    ...
    it('should update project', async () => {
        const res = await request(app)
            ...
            .send({
                ...
                demoUrl: 'https://q23',
                media: [{ type: 'image', url: 'https://q23' }],
