...
const testToken = jwt.sign({ userId: 'cmf16r8iy0000c64sdqbfmb30', username: 'Dehimik' }, JWT_SECRET, { expiresIn: '1h' });
...
describe('POST /api/project/create', () => {
    it('should create a new project', async () => {
        const res = await request(app).post('/api/project/create').set('Authorization', `Bearer ${testToken}`).send({
            title: 'Test Project2',
    ...
    it('should return 400 if userId is missing (simulate by tampered token)', async () => {
        ...
        const res = await request(app).post('/api/project/create').set('Authorization', `Bearer ${tamperedToken}`).send({
            title: 'Good Title',
            description: 'This is a test project',
        });
    ...
    it('should return 400 if project title is not unique per user', async () => {
        await request(app).post('/api/project/create').set('Authorization', `Bearer ${testToken}`).send({
            title: 'UniqueTitle',
            description: 'This is a test project',
        });
        const res = await request(app).post('/api/project/create').set('Authorization', `Bearer ${testToken}`).send({
