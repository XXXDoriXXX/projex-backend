...
import { isValidUrl } from "./urlValid.js";
...
export const projectFieldValid = async (data: CreateProjectData) => {
	...
	if (data.githubUrl) {
		...
		if (data.githubUrl.length > 150)
			throw new Error("GitHub URL length exceeds 150 characters");
		if (!isValidUrl(data.githubUrl)) throw new Error("GitHub URL is invalid");
		if (!data.githubUrl.startsWith("https://github.com/"))
			throw new Error("GitHub URL must be a GitHub link");
	}

	// demo
	if (data.demoUrl) {
		if (typeof data.demoUrl !== "string")
			throw new Error("Demo URL must be a string");
		if (data.demoUrl.length > 150)
			throw new Error("Demo URL length exceeds 150 characters");
