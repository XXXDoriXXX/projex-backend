...
export const getProjectById = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const project = await service.getProjectById(projectId, token, userId);
    if (!project) {
...
export const changeProjectVisibility = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw new ForbiddenError('Authentication required');
    }
    ...
    const updated = await service.changeProjectVisibility(projectId, userId, visible);
    res.status(200).json({ success: true, data: updated, message: 'Visibility updated' });
});

export const likeProject = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw new ForbiddenError('Authentication required');
    }
...
export const unlikeProject = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const userId = req.user?.userId;
    if (!userId) {
        throw new ForbiddenError('Authentication required');
    }
...
export const recordProjectView = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const projectId = req.params.id;
    if (!projectId) {
        throw new ValidationError('Project ID is required', 'id');
    }
